<?php
    ...
    **/
    ...
    /** Controller f퇾 die Administration der 땈ungen **/
    class UebungController extends StudipController
    {
         ...
         * Eigenschaft "flash" h뎝disch initialisiert werden, damit persistent die Werte
         * 퇬ergeben werden k늧nen
         **/
        function before_filter( &$action, &$args )
        {
            ...
            try {
                ...
                // falls keine 땈ungsID gesetzt ist, versuchen die erste 땈ung zu finden, wenn nicht mit Exception abbrechen
                if (Request::quoted("ueid"))
                {
                    $this->flash["uebung"] = new Uebung($this->flash["veranstaltung"], Request::quoted("ueid"));
                    PageLayout::setTitle(_($_SESSION["SessSemName"]["header_line"]. " - Punkteverwaltung - 땈ung [".$this->flash["uebung"]->name()."]"));
                    return;
                } else {
        ...
        function index_action()
        {
            ...
            // setze Variablen (URLs) f퇾 die entsprechende Ajax-Anbindung, falls keine 땈ungsID gesetzt ist nehmen wir die Default Einstellung
        ...
        /** setzt die Einstellungen f퇾 die 땈ung **/
        function updatesetting_action()
        {
            try {
                if (!VeranstaltungPermission::hasDozentRecht($this->flash["uebung"]->veranstaltung()))
                    $this->flash["message"] = Tools::createMessage( "error", _("Sie haben nicht die erforderlichen Rechte um die 땈ung zu ver뎝dern") );
                elseif (Request::submitted("submitted"))
                {
                    ...
                    $this->flash["message"] = Tools::createMessage( "success", _("Einstellung der 땈ung ge뎝dert") );
                }
        ...
        function delete_action()
        {
            try {
                ...
                {
                    if (!VeranstaltungPermission::hasDozentRecht($this->flash["uebung"]->veranstaltung()))
                        $this->flash["message"] = Tools::createMessage( "error", _("Sie haben nicht die erforderlichen Rechte um die 땈ung zu l늮chen") );
                    else
                        Uebung::delete( $this->flash["uebung"] );

                    $this->flash["message"] = Tools::createMessage( "success", _("땈ung gel늮cht") );
                    $this->redirect("admin");
                    return;
                }
                elseif (Request::int("dialogno")) { }
                    ...
                    $this->flash["message"] = Tools::createMessage( "question", _("Soll die 땈ung inkl aller Punkte gel늮cht werden?"), array(), $this->url_for("uebung/delete", array("ueid" => $this->flash["uebung"]->id())) );
        ...
        function jsonchildlist_action()
        {
            ...
            // Daten f퇾 das Json Objekt holen und ein Default Objekt setzen
        ...
        /** liefert die korrekten Json Daten f퇾 den jTable **/
        function jsonlist_action()
        {
            ...
            try {
                ...
                // hole die 땈ung und pr퇰e die Berechtigung (in Abh뎝gigkeit des gesetzen Parameter die 땈ung initialisieren)
                ...
                if ($laData)
                {
                    ...
                    // hole Query Parameter, um die Datenmenge passend auszuw뎗len
                    $laData = array_slice($laData, Request::int("jtStartIndex"), Request::int("jtPageSize"));
                    

                    foreach( $laData as $item )
                    {
                        // siehe Arraykeys unter views/uebung/jsonlist.php & alle String m퇿sen UTF-8 codiert werden, da Json UTF-8 ist
                        $laItem = array(
        ...
        /** erzeugt das Update, f퇾 den jTable **/
        function jsonupdate_action()
        {
            ...
            try {
                ...
                // hole die 땈ung und pr퇰e die Rechte
                if ( (!VeranstaltungPermission::hasTutorRecht( $this->flash["uebung"]->veranstaltung() )) && (!VeranstaltungPermission::hasDozentRecht( $this->flash["uebung"]->veranstaltung() )) )
                    throw new Exception(_("Sie haben nicht die notwendige Berechtigung"));

                // hole die Zuordnung von 땈ung und Student und setze die Daten
                ...
                // alles fehlerfrei durchlaufen, setze Result (lese die ge뎝derten Daten aus der Datenbank)
                $this->result["Result"] = "OK";
        ...
        function massedit_action()
        {
            ...
            try {
                ...
                foreach(explode("\n", Request::quoted("massinput")) as $lcLine)
                {
                    ...
                    {
                        array_push($laError, _("Zeile ".$i." hat ein ung퇷tiges Format"));
                        continue;
                    }
